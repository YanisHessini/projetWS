...
<template>
	...
	<div class="mt-5 md:col-span-2 md:mt-0">
		<div class="flex flex-col justify-center items-center mt-10">
			<h1 class="text-4xl font-bold text-slate-100 p-6 rounded-xl shadow bg-black ">Réserver</h1>
		</div>
		<br />
		<div class="overflow-hidden shadow sm:rounded-md">
			<div class="flex flex-col justify-center items-center mt-10">
				<h3 class="text-2xl font-bold text-gray-800">Train : {{ train.trainJson.departure_station }} - {{
					train.trainJson.arrival_station
				}}</h3>
			...
			<div class="overflow-hidden shadow sm:rounded-md">
				<div class="bg-white px-4 py-5 sm:p-6">
					<div class="grid grid-cols-6 gap-6">
						...
						<div class="col-span-6 sm:col-span-3">
							<label for="class" class="block text-sm font-medium text-gray-700">Sélectionnez votre classe</label>
							<select id="class" name="class"
								class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-2 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
								<option value="1">Classe 1 : {{ train.trainJson.nb_class_1 - train.trainJson.current_nb_class_1 }}
									places restantes au prix de {{ train.trainJson.price_class_1 }} euros</option>
								<option value="2">Classe 2 : {{ train.trainJson.nb_class_2 - train.trainJson.current_nb_class_2 }}
									places restantes au prix de {{ train.trainJson.price_class_2 }} euros</option>
								<option value="3">Classe 3 : {{ train.trainJson.nb_class_3 - train.trainJson.current_nb_class_3 }}
									places restantes au prix de {{ train.trainJson.price_class_3 }} euros</option>
							</select>
						</div>
					</div>
				</div>
				<div class="bg-gray-50 px-4 py-3 text-center sm:px-6">
					<button type="submit"
						class="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Réserver</button>
				</div>
			</div>
		</form>
	</div>
